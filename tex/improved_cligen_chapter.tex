...
very different from that of the other three combinations.
This combination (v5.2 + updated) showed relatively high mean monthly maxima of
daily peak intensity during the summer months (June, July and August) with a
distinctively high peak in August (Figure
...
considered essential. Thus, WEPP was calibrated by adjusting hydrological and
...
(Figure \ref{fig:calibration}). Then, in order to find possible effects of
changes ininputs, we may change \emph{Input A} to \emph{Input A'}. When the
model was run with \emph{Input A'}, the responding model output should be
\emph{Output B'} if the model has a linear relationship between inputs and
outputs. However, because of the non-linear relationship, the responding output
may be rather unknown \emph{Output B''} (Figure \ref{fig:calibration}). This
means that, unless model inputs and outputs are identified and the model is
calibrated against known output values, it may be difficult to measure the
extent of changes in model predictions. This is because we may not know where
unknown \emph{Output B''} has arrived from. Therefore, it is very unlikely that
non-calibrated models will lead to reasonable simulation results.
...
configurations (v4.1 + original and v4.1 + updated). Mean annual runoff and soil
...
this research when simulation results are presented. Also, calibrations carried
...
with responding rainfall intensity, that is high rainfall intensity, hence short
rainfall durations. This is more evident as new CLIGEN (v5.2) generated shorter
annual rainfall durations with original input file (Figure
...
that the improvement of CLIGEN has less impacts on weather generations for
...
Therefore, it is strongly suggested to use new CLIGEN (v5.2) hereafter. It is
also advisable to review previous CLIGEN input files when possible.
...
\citet{baffaut1996-447} used old CLIGEN to generate 200 years of climate data
for WEPP simulations in order to investigate impacts of CLIGEN parameters on
WEPP-predicted average annual soil loss. The study found that only five
parameters, that are mean precipitation per event, standard deviation of the
precipitation per event, skewness of the precipitation per event, probability of
a wet day following a wet day and probability of a wet day following a dry day,
influenced the estimated average annual soil loss. However, they found that half
hour largest intensity (i.e. {MX.5P}) and the statistical distribution of the
time to peak had less influence on the output. Insensitivity to {MX.5P}
parameter, which \citet{baffaut1996-447} found, could be the result of the error
found in old CLIGEN as \citet{yu2000-301} pointed out.
...
and eastern United States \citet{ashley2003-3003}. This means that the magnitude
...
\citet{zhang1996-855} conducted a study to evaluate the overall performance of
...
study may not have been affected by the change of CLIGEN. As this chapter
suggested, rainfall duration and intensity are the most affected parameters in
WEPP climate inputs.

\citet{baffaut1998-756} also used old CLIGEN to generate synthetic weather
...
of the high rainfall intensity for their study sites \citet{ashley2003-3003}.
...
\citet{favis-mortlock1999-329} conducted a case study from Brazil to suggest an
...
CLIGEN is now sensitive to changes of rainfall intensity which is parametrized
