...
\section{Background and Data Availability}
\label{sec:Data}

All of the datasets used in this research; for climate analyses, model
parametrizations and erosion simulations are obtained from the South Downs,
England, UK. Figure \ref{fig:DailyRainfallDataSite} shows locations of
rainfall gauges and types of rainfall data recorded.

One of the reasons for choosing this particular research site is that data
availability of the site is reasonably good. The area has been extensively
monitored for soil erosion since the late 1970s
\citep{boardman1995-177,boardman2003-176}. In addition, there is a
well-established expertise about the area that can be referenced to the current
research \citep{boardman1995-177,favis-mortlock1995-365,favis-mortlock1997-79,
favis-mortlock1998-141,boardman2001-346,boardman2003-176}.

\citet{boardman2003-176} provides a comprehensive insight of soil erosion
problems in the region as well as the details of researches that have been
carried out previously. A compilation of rainfall events and subsequent soil
erosion events are included in the article.

With the exception of rainfall data, the data used here for erosion modelling
are adopted from \citet{favis-mortlock1998-141}. Details of these data are
included in this chapter.

Another more important reason for choosing Woodingdean site as a research
site is because observed (but rather short-term) soil loss data are available
for this site (Table \ref{tab:ObservedSoilLoss}). These observed soil loss data
are used later for the model calibration (See page
\pageref{sec:RunoffAndSoilLossSimulation}).

\begin{table}[htbp]
  \figureversion{tabular}
  \centering
  \caption[Observed annual average soil loss from Woodingdean]{Observed
annual average soil loss from hillslopes (D-L) of Woodingdean}
  \label{tab:ObservedSoilLoss}
  \footnotesize
...
\section{Definitions of Intensity and Rainfall Storm}
...
acquired initially from the site (Table
\ref{tab:PrecipitationDataUsedForCurrentRainfallTrendInvestigation}): monthly,
daily and event data.
Details of these datasets are described here because they have been used at a
certain stage of current research. For example, monthly grid and daily station
data are used for the investigation on the trend of rainfall intensity in the
area---included in Appendix \ref{sec:RainfallCharacteristicsOfTheStudyArea}. As
pointed out previously, attempts to find a trend of rainfall intensity did
not succeed. However, because these data were used in this research at some
points of the research procedure, their details are give here.

For the weather inputs of erosion model simulations, only event rainfall
datasets are used (Table \ref{tab:DetailsOfDataStations}). This is because
soil erosion models require high resolution rainfall data such as tipping
bucket rainfall data. Event rainfall datasets are used either as original
rainfall data or as parametrised rainfall data for erosion estimations.

For few occasions, design storm data were also used for erosion simulations (See
Chapter \ref{sec:EFFECTSOFCONTINOUSANDDISCONTINUSSTORM} and Chapter
\ref{sec:EFFECTSOFRAINFALLINTENSITYCHANGESONSOILEROSION}). The reasons for
using design storm data and data themselves are presented at the chapter where
design storms are used.
...
intensity\\
    \bottomrule
%   \addlinespace[1mm]
    \multicolumn{4}{p{12cm}}{\footnotesize $^\dagger$ Aggregated into
1-min data which are originally from tipping bucket data}
    \end{tabular}
\end{table}
...
\ref{tab:DetailsOfDailyDataStations}). Locations of individual daily stations
are shown in Figure \ref{fig:DailyRainfallDataSite}.

\begin{table}[htbp]
  \figureversion{tabular}
  \centering
  \caption{Details of daily rainfall data stations}
  \label{tab:DetailsOfDailyDataStations}
    \small
    \begin{tabular}{lllll}
      \toprule
...
%\ref{tab:DetailsOfDailyDataStations} and Table \ref{tab:DetailsOfDataStations},
%respectively.
...
\begin{table}[htbp]
  \figureversion{tabular}
  \centering
  \caption{Details of event rainfall data stations}
  \label{tab:DetailsOfDataStations}
    \small
    \begin{tabular}{lllll}
      \toprule
      \textbf{Code} & \textbf{Station} & \textbf{Data type} & \textbf{Grid
reference} & \textbf{Periods}\\
      \midrule
%      DR & Ditchling Road & daily & TQ314076 & 1980--1989\\
%      SO & Southover & daily & TQ407093 & 1980--1998\\
%      PL & Plumpton & daily & TQ356136 & 1980--2000\\
...
%      HD & Housedean & daily & TQ369093 & 1967--2004\\
%      FF & Falmer Farm & daily & TQ342084 & 1904--2002\\
      S & Southover & event & TQ407093 & 1993--2001$^\dagger$\\
      D & Ditchling Road & event & TQ315077 & 1991--2003$^\ddagger$\\
...
\multicolumn{6}{l}{$^\ddag$ Average intensity calculated with effective
durations.}
\end{tabular}
